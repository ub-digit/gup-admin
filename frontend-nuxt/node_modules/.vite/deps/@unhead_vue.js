...
var resolveTitleTemplate = (template, title) => {
  if (template == null)
    return title || null;
...
var setAttrs = (ctx, newEntry = false, markSideEffect) => {
  const { tag, $el } = ctx;
  if (!$el)
    return;
    ...
    hooks: {
      "tag:normalise": (ctx) => {
        ...
        if (!HasElementTags.includes(tag.tag) || !tag.key)
          return;
        if (IsBrowser || ((_b = (_a = getActiveHead()) == null ? void 0 : _a.resolvedOptions) == null ? void 0 : _b.document))
          return;
...
// node_modules/@unhead/vue/dist/shared/vue.3c82dacb.mjs

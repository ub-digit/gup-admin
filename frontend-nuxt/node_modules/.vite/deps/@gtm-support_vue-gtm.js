import {
  nextTick
} from "./chunk-467QAF56.js";
import "./chunk-IXINTZTP.js";
import {
  __publicField
} from "./chunk-IRT3OBIB.js";
...
function loadScript(id, config) {
  ...
  const scriptLoadListener = (event) => {
    var _a22;
    (_a22 = config.onReady) == null ? void 0 : _a22.call(config, { id, script });
    script.removeEventListener("load", scriptLoadListener);
  };
  script.addEventListener("load", scriptLoadListener);
  window.dataLayer = (_a2 = window.dataLayer) != null ? _a2 : [];
  ...
  const queryString = new URLSearchParams({
    id,
    ...(_b = config.queryParams) != null ? _b : {}
  });
  const source = (_c = config.source) != null ? _c : "https://www.googletagmanager.com/gtm.js";
  script.src = `${source}?${queryString}`;
  const parentElement = (_d = config.parentElement) != null ? _d : doc.body;
...
var GtmSupport = class {
  ...
  dataLayer() {
    ...
    if (this.isInBrowserContext() && this.options.enabled) {
      return window.dataLayer = (_a = window.dataLayer) != null ? _a : [];
    }
    return false;
  }
  trackView(screenName, path, additionalEventData = {}) {
    var _a, _b, _c;
    const trigger = this.isInBrowserContext() && ((_a = this.options.enabled) != null ? _a : false);
    ...
    if (trigger) {
      const dataLayer = window.dataLayer = (_b = window.dataLayer) != null ? _b : [];
      dataLayer.push({
        ...additionalEventData,
        event: (_c = this.options.trackViewEventProperty) != null ? _c : "content-view",
        "content-name": path,
  ...
  trackEvent({
    ...
    if (trigger) {
      const dataLayer = window.dataLayer = (_b = window.dataLayer) != null ? _b : [];
      dataLayer.push({
        event: event != null ? event : "interaction",
        target: category,
        action,
...
function initVueRouterGuard(app, vueRouter, ignoredViews = [], trackOnNextTick, deriveAdditionalEventData = () => ({})) {
  ...
  vueRouter.afterEach(async (to, from, failure) => {
    ...
    const baseUrl = (_a2 = (_c = (_b = vueRouter.options) == null ? void 0 : _b.history) == null ? void 0 : _c.base) != null ? _a2 : "";
    let fullUrl = baseUrl;
    if (!fullUrl.endsWith("/")) {
      fullUrl += "/";
    }
